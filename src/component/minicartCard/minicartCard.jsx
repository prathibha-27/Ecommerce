...
function MinicartCard({ name, id, image, qty, price, stock, error, setError }) {
  ...
  const handleBlur = (e) => {
    const q = cartItems.map((item) => {
      if (item.id && item.id === id && (!quantity || quantity <= 0)) {
        return { ...item, qty: 1 };
  ...
  return (
    <div className="minicartCard">
      <ul className="minicartCard-list">
        <li className="minicartCard-list-products">
          <img src={image} />
          <div className="minicartCard-list-products-details">
            ...
            <div className="minicartCard-list-products-details-actions">
              <input
                type="number"
                value={quantity || ""}
              ...
              <svg
                className="cartcard-content-btn"
            ...
            <div>
              {error && quantity > stock ? (
                ...
                >
                  please add qty within stock availability
                </p>
              ) : null}
            </div>
          </div>
